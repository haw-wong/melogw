...
def tabMach_arr(tab):
    with tab:
        col1, col2, col3, col4, col5 = st.columns([0.2,0.3,0.3,0.1,0.1])
        with col1:
        ...
        if selected=="Main Engine":
            dfME = pd.read_excel("erlb.xlsx",sheet_name="mainengine",
                            usecols='X:AP', skiprows=range(23), # Skip rows before the start
                            nrows=32 - 23)  # Read only the specified number of rows
